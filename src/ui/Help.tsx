...
export const PIE_RULE_HELP = (
  <>
    ...
    <p>1. そのまま自分の手を打つ。<br />2. <b>スワップ</b>: 白の 1 手目を「奪う」。このアプリでは色は交換せず、駒を盤の対角線(A1–X24)で折り返した位置に移して赤の駒にします。列と行の入れ替えなので、例えば白の F3 は赤の C6 になります(白にとっての上辺からの距離が、赤にとっての左辺からの距離になる)。そのあと白が続けて打ちます。</p>
...
const PATTERNS: Pattern[] = [
  { name: "1 つ飛ばし", offset: [0, 2], summary: "縦(横)に、間に穴を 1 つあけて並んだ 2 駒", detail: "間の穴をはさんで左右(上下)に中継点が 1 つずつ。短い距離を確実につなぎたいときの形です。" },
  { name: "斜め隣", offset: [1, 1], summary: "斜めに隣り合う 2 駒", detail: "中継点が 2 か所あるので、相手が片方を塞いでももう片方でつながります。最も手堅い形です。" },
  { name: "3 つ飛ばし", offset: [0, 4], summary: "縦(横)に、間に穴を 3 つあけて並んだ 2 駒", detail: "中継点は真ん中の行の左右に 1 つずつ。まっすぐ遠くまで伸ばしたいときの形で、相手の妨害に強いです。" },
  { name: "横 1 ずれ・縦 2 つ飛ばし", offset: [1, 3], summary: "横に 1 列ずれ、縦に間の穴 2 つ分離れた 2 駒", detail: "L 字のような形。中継点は 2 か所あり、進む方向を少しずらしながら伸ばせます。" },
  { name: "斜め 2 つ飛ばし", offset: [3, 3], summary: "斜めに、間に穴を 2 つあけて並んだ 2 駒", detail: "中継点は 2 か所。斜めに大きく進める形で、盤の広い範囲を一気にカバーします。" },
];
